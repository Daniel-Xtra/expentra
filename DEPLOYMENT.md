...
Deploy always recreates `api`/`worker` to match the new `sha-<commit>` after a successful migrate. If migrate/postgres fails **before** recreate, env is reverted to the still-running image so you never get “env new / container old”. After recreate, health failures keep the new image and fail the job (no silent rollback to an older SHA).
